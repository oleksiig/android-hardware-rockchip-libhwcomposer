...
void DrmConnector::SetDpmsMode(uint32_t dpms_mode) {
	ALOGD("SetDpmsMode: dpms_property_.id()=%d, dpms_mode=%d", dpms_property_.id(), dpms_mode);
...
void DrmConnector::set_best_mode(const DrmMode &mode) {
  best_mode_ = mode;
	...
	ALOGD("set_best_mode: h=%d,v=%d,interlaced=%d,v_refresh_=%f,flags=%d,clk=%d",
         mode.h_display(), mode.v_display(), mode.interlaced(), mode.v_refresh(), mode.flags(), mode.clock());
}

void DrmConnector::set_active_mode(const DrmMode &mode) {
  active_mode_ = mode;
  
  ALOGD("set_active_mode: h=%d,v=%d,interlaced=%d,v_refresh_=%f,flags=%d,clk=%d",
         mode.h_display(), mode.v_display(), mode.interlaced(), mode.v_refresh(), mode.flags(), mode.clock());
}
